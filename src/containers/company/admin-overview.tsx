...
import { AdminsTable } from "@components/tables/company/admins-table";
import { useUpdateSearchParam } from "@hooks/context/useSearchParams";
import { usePrefetchQueryData } from "@hooks/prefetch-query-data";
import { CreateAdmin } from "@layouts/modal-layout/create-admin";
import { UserPlusIcon } from "@phosphor-icons/react";
import { LIMIT } from "@services/api-service";
import { getAllAdminsQueryOptions } from "@services/tanstack-queries/staff-management";
...
export const AdminOverview = ({ search }: AdminOverviewProps) => {
  const { isOpen, onOpen, onClose } = useDisclosure();

  const updateSearchParam = useUpdateSearchParam<OverviewSearchType>();
  const { prefetch } = usePrefetchQueryData(getAllAdminsQueryOptions);

  const { data: adminResponse, isLoading } = useQuery({
    ...getAllAdminsQueryOptions(search),
    ...
    <>
      <VStack align="stretch" spacing={6}>
        <Flex
          ...
          <Box>
            <Heading size="lg">Admins network</Heading>
      ...
      <CreateAdmin isOpen={isOpen} onClose={onClose} />
    </>
  );
};
